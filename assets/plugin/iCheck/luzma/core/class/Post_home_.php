...
class Posts_home extends  Gurisha {
    ...
    public function tweets($user_id,$limit)
    {
        ...
        $sql="SELECT * FROM tweets LEFT JOIN users ON tweetBy= user_id WHERE tweetBy = $user_id AND retweet_id='0' OR tweetBy= user_id AND retweet_by != $user_id AND tweetBy IN (SELECT receiver FROM follow WHERE sender= $user_id) ORDER BY tweet_id DESC LIMIT 0,5 ";
    ...
    <?php 
        ...
        $mysqli= $this->database;
        $sql="SELECT * FROM tweets LEFT JOIN users ON tweetBy= user_id WHERE tweetBy = $user_id AND retweet_id='0' OR tweetBy= user_id AND retweet_by != $user_id AND tweetBy IN (SELECT receiver FROM follow WHERE sender= $user_id) ORDER BY tweet_id DESC LIMIT 5,5 ";
        $query= $mysqli->query($sql);
        $tweets=array();
        while ($row= $query->fetch_assoc()) {
            # code...
    ...
    <?php 
        ...
        $mysqli= $this->database;
        $sql="SELECT * FROM tweets LEFT JOIN users ON tweetBy= user_id WHERE tweetBy = $user_id AND retweet_id='0' OR tweetBy= user_id AND retweet_by != $user_id AND tweetBy IN (SELECT receiver FROM follow WHERE sender= $user_id) ORDER BY tweet_id DESC LIMIT 10,5 ";
        $query= $mysqli->query($sql);
        $tweets=array();
        while ($row= $query->fetch_assoc()) {
            # code...
