...
+function ($) {
  ...
  var Selector = {
    ...
    controlSidebar_home: '.control-sidebar-home',
    fixed         : '.fixed',
  ...
  Layout.prototype.fixSidebar = function () {
    ...
    if (this.options.slimscroll) {
      if (typeof $.fn.slimScroll !== 'undefined') {
        ...
        // Add slimscroll
        if ($(Selector.controlSidebar).find(Selector.slimScrollDiv).length === 0) {
          $(Selector.controlSidebar_home).slimScroll({
            height: ($(window).height() - $(Selector.mainHeader).height() - $(Selector.mainFooter).height()) + 'px'
          });
        }
      }
    }
