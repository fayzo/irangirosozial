...
class Users{
    ...
    public function jobsPosterlogin($job_user,$username,$email,$password,$datetime)
    {
       ...
       $sql= $mysqli->query("SELECT user_id,username FROM users WHERE username ='{$email}' AND password='{$password}' OR email ='{$email}'and password='{$password}' ");
       $sql1= $mysqli->query("SELECT user_id ,username FROM users WHERE username ='{$email}' or email ='{$email}'");
